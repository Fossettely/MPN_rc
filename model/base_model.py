    ...
    def __init__(self, t_length = 5, n_channel =3):
        ...
        def Basic(intInput, intOutput):
            return torch.nn.Sequential(
                ...
                torch.nn.BatchNorm2d(intOutput),  # add
        ...
        # 将第二个卷积换成sspcab,没有考虑该块损失
        def SSPCAB_Blk(intInput, intOutput):
            return torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=intInput, out_channels=intOutput, kernel_size=3, stride=1, padding=1),
                torch.nn.ReLU(inplace=False),
                # torch.nn.Conv2d(in_channels=intOutput, out_channels=intOutput, kernel_size=3, stride=1, padding=1),
        ...
        self.moduleBatchNorm = torch.nn.BatchNorm2d(512)  # add
        self.moduleReLU = torch.nn.ReLU(inplace=False) # add
    ...
    def __init__(self, t_length = 5, n_channel =3):
        ...
        def Basic_(intInput, intOutput):
            return torch.nn.Sequential(
                ...
                torch.nn.ReLU(inplace=False),
                # torch.nn.Conv2d(in_channels=intOutput, out_channels=intOutput, kernel_size=3, stride=1, padding=1),
                SSPCAB(intOutput),    #  倒数第一个卷积替换为sspcab而非文中第二个
                torch.nn.ReLU(inplace=False)
            ) 
        ...
        # self.moduleConv = Basic(1024, 512)  #  add
        self.moduleUpsample4 = Upsample(512, 256)

        self.moduleDeconv3 = Basic(512, 256)
        self.moduleUpsample3 = Upsample(256, 128)

        self.moduleDeconv2 = Basic(256, 128)   # 原始
        # self.moduleDeconv2 = Basic_(256, 128)  # 替换为sspcab
