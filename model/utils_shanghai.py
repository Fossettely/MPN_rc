...
class DataLoader(data.Dataset):
    ...
    def get_all_samples(self):
        frames = []
        videos = glob.glob(os.path.join(self.dir, '*'))
        for video in natsorted(videos):  # different with other dataset ,shanghai is natsorted rather sorted
            video_name = video.split('/')[-1]
...
class VideoDataLoader(data.Dataset):
    def __init__(self, video_folder, dataset_type, transform, resize_height, resize_width, time_step=4, segs=32,
                 num_pred=1, batch_size=1):
        self.dir = video_folder
...
class MetaDataLoader(data.Dataset):
    def __init__(self, video_folder, transform, resize_height, resize_width, time_step=4, task_size=2, segs=32,
                 num_pred=1):
...
# train_folder = '../data/shanghai/training/frames/'
...
# train_batch = data.DataLoader(train_dataset, batch_size=1, shuffle=True, num_workers=0, drop_last=True)
...
#         imgs = imgs.view(1, -1, imgs.shape[-2], imgs.shape[-1])       #  image in shanghai batch_size must be 1